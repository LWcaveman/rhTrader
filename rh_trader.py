...
from monitor import send_setup_alert  # Requires the monitor.py update from our earlier step
...
def scan_and_evaluate():
    ...
    # Maximum capital logic
    max_alloc_pct = const.MAX_ALLOCATION_PCT_MICO if total_equity < const.ACCOUNT_100 else const.MAX_ALLOCATION_PCT
    max_capital_per_trade = total_equity * max_alloc_pct

    print("\n" + "=" * 65)
    print(f"DAILY SWING SCANNER - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Account Equity: ${total_equity:.2f} | Buying Power: ${buying_power:.2f}")
    print(f"Risk Budget ({const.DEFAULT_RISK_PER_TRADE*100}%): ${total_equity * const.DEFAULT_RISK_PER_TRADE:.2f} | Max Capital per Trade: ${max_capital_per_trade:.2f}")
    print("=" * 65)

    # Setup session for batch yfinance download
    ...
    for ticker in watchlist:
        try:
            # Handle multi-ticker vs single-ticker download shapes
            df = data[ticker].dropna().copy() if len(watchlist) > 1 else data.dropna().copy()
            ...
            is_engulfing = (prev_close < prev_open and open_p <= prev_close and close > prev_open)

            body = abs(close - open_p)
            lower_wick = min(open_p, close) - low
            upper_wick = high - max(open_p, close)
            is_hammer = (lower_wick >= 2 * body) and (upper_wick <= 0.2 * (high - low))

            triggered = is_engulfing or is_hammer
            ...
            if is_uptrend and retested_support and triggered:
                ...
                # Position Sizing & Cash Guard dynamically calculated from total_equity
                ...
                trigger_type = "Hammer" if is_hammer else ("Engulfing" if is_engulfing else "Breakout")
                ...
                # Dispatch alert to Discord immediately
                try:
                    send_setup_alert(setup_data)
                    print(f"[Scanner] Found setup for {ticker} ({trigger_type}) -> Alerted.")
                except NameError:
                    print(f"[Scanner] Found setup for {ticker} ({trigger_type}) -> Logged.")

        except Exception:
            # Skip tickers that throw data calculation errors
            continue
    ...
    if active_info["count"] > 0:
        print(f"Active Live Positions ({active_info['count']}): {', '.join(active_info['tickers'])}")
